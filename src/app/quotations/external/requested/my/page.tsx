...
export default function MyRequestedQuotationsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Check both userEmail field and action history entries
        let myQuotations = data.filter(quotation => {
          // Check if userEmail field matches current user
          if (quotation.userEmail === email) {
            return true;
          }
          
          // Check if user email appears in action history
          return quotation.actionHistory.some(action => 
            action.toLowerCase().includes(email.toLowerCase())
          );
        });
        ...
        if (canViewOnlyCompanyData && userCompany) {
          ...
          console.log(`Filtering "My" quotation requests by company ID "${companyId}" (before: ${myQuotations.length} records)`);
          myQuotations = myQuotations.filter(q => q.company === companyId);
          console.log(`After filtering: ${myQuotations.length} records match user's company`);
        } else {
          console.log(`Not filtering by company. Showing all ${myQuotations.length} of user's quotation requests`);
        }
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  View quotation requests you have submitted
                </CardDescription>
              </div>
